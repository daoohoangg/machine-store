...
const defaultNews = [
  {
    ...
    content: `
      ...
      <p>Năm nay, các dòng máy từ Honda (Nhật Bản), Oshima và Kama vẫn đang dẫn đầu thị trường. Dòng Honda GX-35 với động cơ bền bỉ và độ rung thấp là lựa chọn hàng đầu cho chị em phụ nữ làm vườn. Kế đến là máy xới đất động cơ dầu Kama với lực phay mạnh mẽ, chuyên trị các loại đất cứng, đất đồi.</p>
  ...
  },
  {
    ...
    content: `
      ...
      <p>Panasonic và Pentax (Italy) là hai thương hiệu bảo chứng cho chất lượng và độ bền. Ngoài ra đối với máy bơm ruộng, dòng bơm xăng Oshima là lựa chọn cơ động vì không phụ thuộc vào nguồn điện lưới.</p>
    `
  },
  {
    ...
    link: '#',
    content: `
      <h2>Tổng quan về máy phát điện Oshima</h2>
      <p>Với chất lượng Nhật Bản và giá thành dễ tiếp cận, hãng máy Oshima được mệnh danh là hàng "quốc dân" tại thị trường Việt Nam. Gần đây, dòng máy phát điện của hãng liên tục cháy hàng mỗi dịp Hè về khi tình trạng cúp điện diễn ra thường xuyên.</p>
      <img src="https://images.unsplash.com/photo-1581092160562-40aa08e78837?auto=format&fit=crop&w=800&q=80" alt="Máy phát điện Oshima" style="max-width: 100%; border-radius: 8px; margin: 20px 0;" />
      ...
      <h2>Kết luận</h2>
      <p>Nếu bạn đang tìm kiếm một tổ máy để backup năng lượng cho gia đình, cửa hàng quy mô nhỏ, máy phát điện Oshima 3kw – 5kw là lựa chọn hoàn hảo cả về giá trị lẫn độ bền theo thời gian.</p>
    `
  },
  {
  ...
  },
  {
    ...
    content: `
      ...
      <p>Các nhãn hàng nổi tiếng trong phân khúc này bao gồm Pegasus, Wing, Oshima đều đang được phân phối chính hãng tại Điện Máy Tuấn Minh.</p>
    `
  },
  {
    ...
    content: `
      ...
      <p>Vì thương hiệu quá nổi tiếng, hiện có hàng trăm xưởng gia công đang "chế" lại vỏ máy giống 90% các dòng máy xịt thuốc, cưa xích của Oshima. Máy giả thường dùng kim loại tái chế, bộ nong dỏm, rất mau hỏng sau 3 - 4 tháng sử dụng.</p>
      <img src="https://images.unsplash.com/photo-1530124566582-a618bc2615dc?auto=format&fit=crop&w=800&q=80" alt="Phân biệt thật giả máy Oshima" style="max-width: 100%; border-radius: 8px; margin: 20px 0;" />
      <h2>2. Ba điểm then chốt để kiểm tra</h2>
      <ol>
        <li><strong>Dập nổi logo:</strong> Trên các nòng máy và sườn lốc máy của hàng thật, biểu tượng Oshima được đúc khối, dập nổi kim loại cực kì sắc sảo. Hàng giả thường dán decal hoặc in phun lem nhem.</li>
        ...
        <li><strong>Nghe tiếng máy:</strong> Máy Oshima thật khi kéo ga lên tiếng gầm giòn giã, dứt khoát chứ không bị nghẹt xả hay rè rền âm kim loại.</li>
      </ol>
...
export const useNews = () => {
  const newsList = useState('site-news', () => JSON.parse(JSON.stringify(defaultNews.map(n => ({
    ...n,
  ...
  const loadNews = () => {
    if (import.meta.client && !isLoaded.value) {
      setTimeout(() => {
        try {
          const saved = localStorage.getItem('site_news_v2')
          if (saved) {
            const parsed = JSON.parse(saved)
            if (Array.isArray(parsed) && parsed.length > 0) {
              // Ensure legacy items have slug and content
              newsList.value = parsed.map(item => ({
                ...item,
  ...
  const saveNews = () => {
    if (import.meta.client) {
      localStorage.setItem('site_news_v2', JSON.stringify(newsList.value))
    }
  }
  ...
  const updateNews = (index: number, item: any) => {
