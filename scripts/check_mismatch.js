...
async function check_mismatch(cid) {
  try {
    const res = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ limit: 20, page: 1, cat_id: cid })
    });
    const data = await res.json();
    const products = data.data.products || [];
    console.log(`Requests cat_id ${cid}. Results:`);
    products.forEach((p, i) => {
        if (i < 10) console.log(`- ${p.id}: ${p.name} (cat_id: ${p.cat_id})`);
    });
  } catch (e) { console.error(e); }
}

check_mismatch(388187);
